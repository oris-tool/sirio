...
/**
 * A transition feature specifying a marking-dependent rate.
 */
public final class RateExpressionFeature implements TransitionFeature {
    ...
    /**
     * Creates a rate with the given expression.
     * @param expr expression
     */
    public RateExpressionFeature(String expr) {
        expression = expr;
    ...
    /**
     * Returns the rate value for a given marking.
     *
     * @param n Petri net
     * @param m marking
     * @return the rate value for the marking
     */
    public double getRate(PetriNet n, Marking m) {
        ...
        if (!res.isNumeric()) {
            throw new IllegalStateException("Rates must resolve to a numeric value."
                    + " Expression: " + expression + " - Marking: " + m);
        }

        double rec = res.getNumericValueAsReal();
        if (rec < 0.0) {
            throw new IllegalStateException("Rates must be nonnegative."
                    + " Expression: " + expression + " - Marking: " + m);
        }
